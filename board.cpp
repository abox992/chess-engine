...
void Board::makeMove(Move move) {
    ...
    std::memcpy(prevState.pieces, pieces, sizeof(uint64_t) * (end(pieces)-begin(pieces)));
    std::memcpy(prevState.allPieces, allPieces, sizeof(uint64_t) * (end(allPieces)-begin(allPieces)));
    std::memcpy(prevState.canCastle, canCastle, sizeof(bool) * (end(canCastle)-begin(canCastle)));
    prevState.enpassantPos = enpassantPos;
    prevState.empty = empty;
    prevState.blackToMove = blackToMove;
    prevState.halfMoves = halfMoves;
    prevState.fullMoves = fullMoves;
...
void Board::unmakeMove() {
    ...
    // load state
    std::memcpy(pieces, prevState.pieces, sizeof(uint64_t) * (end(pieces)-begin(pieces)));
    std::memcpy(allPieces, prevState.allPieces, sizeof(uint64_t) * (end(allPieces)-begin(allPieces)));
    std::memcpy(canCastle, prevState.canCastle, sizeof(bool) * (end(canCastle)-begin(canCastle)));
    enpassantPos = prevState.enpassantPos;
    empty = prevState.empty;
    blackToMove = prevState.blackToMove;
